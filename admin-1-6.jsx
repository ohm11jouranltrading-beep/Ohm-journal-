...
function ProfilePage({client:c,onBack,onDelete}){
  ...
  const handleDelete=async()=>{
    console.log("[Delete] button pressed. uid=",c.uid,"onDelete typeof=",typeof onDelete);
    if(typeof onDelete!=="function"){
      const m="خطأ برمجي: دالة الحذف (onDelete) غير متصلة بهذا المكوّن.";
      console.error(m); setDelError(m); alert(m); return;
    }
    if(!c.uid){
      const m="خطأ: لا يوجد معرّف (uid) لهذا الحساب، لا يمكن الحذف.";
      console.error(m); setDelError(m); alert(m); return;
    }
    setDeleting(true);setDelError("");
    try{
      await onDelete(c.uid);
      console.log("[Delete] succeeded for uid=",c.uid);
      // success: onDelete unmounts this page by clearing "selected" upstream
    }catch(e){
      console.error("[Delete] failed:",e);
      const m = e?.code==="permission-denied"
        ? "تم رفض العملية من صلاحيات Firestore (Security Rules) — لازم تعدّل قواعد الأمان بـ Firebase Console عشان تسمح للأدمن بحذف مستندات users. افتح Console وابعتلي رسالة الخطأ لو لسا موجودة."
        : "فشل الحذف: "+(e?.message||e?.code||JSON.stringify(e)||"خطأ غير معروف — افتح Console (F12) وشوف تفاصيل الخطأ");
      setDelError(m);
      alert(m); // guaranteed visible even if the inline banner render is missed
      setDeleting(false);
    }
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0C0A08",color:"#fff",fontFamily:"'Outfit',sans-serif",direction:"rtl"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#170F08 0%,#2A1B08 40%,#170F08 100%)",borderBottom:"1px solid rgba(217,119,6,0.1)",padding:"24px 28px",position:"relative",overflow:"hidden"}}>
        ...
        {delError&&(
          <div style={{background:"rgba(239,68,68,0.12)",border:"1px solid rgba(239,68,68,0.35)",borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:14}}>⚠️</span>
            <p style={{fontSize:12,color:"#f87171",fontWeight:600,flex:1}}>{delError}</p>
            <button onClick={()=>setDelError("")} style={{background:"none",border:"none",color:"#f87171",cursor:"pointer",fontSize:14,fontWeight:700}}>✕</button>
          </div>
        )}
        ...
        {tab==="overview"&&(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12}}>
              {kpis.map(k=><KpiTile key={k.label} {...k} onClick={()=>setKpiSel(k)}/>)}
            </div>
...
export default function AdminPage({user,onBack}){
  ...
  async function deleteClient(uid){
    await deleteDoc(doc(db,"users",uid)); // let errors propagate to caller (ProfilePage) for inline display
