...
function EquityCone({trades,w=140,h=48}){
  if(!trades?.length||trades.length<2)return<div style={{width:w,height:h,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:10,color:"rgba(255,255,255,0.15)"}}>—</span></div>;
  const sorted=[...trades].sort((a,b)=>(a.date||"").localeCompare(b.date||""));
  let cum=0;const pts=sorted.map(t=>{cum+=(t.pnl||0);return cum;});
  const mn=Math.min(...pts,0),mx=Math.max(...pts,0),rng=(mx-mn)||1;
  const isPos=pts[pts.length-1]>=0;
  const col=isPos?"#10b981":"#ef4444";
  const n=pts.length;
  const mid=h/2;
  const maxHalf=mid-2,minHalf=2.5;
  const top=[],bot=[],trend=[];
  pts.forEach((v,i)=>{
    const t=i/(n-1);
    const x=t*w;
    // smooth, always-visible cone taper — narrow mouth → wide bell (no noise on the outline itself)
    const half=minHalf+(maxHalf-minHalf)*Math.pow(t,0.55);
    top.push(`${x.toFixed(1)},${(mid-half).toFixed(1)}`);
    bot.push(`${x.toFixed(1)},${(mid+half).toFixed(1)}`);
    // real equity trend rides *inside* the cone as a glowing line
    const norm=(v-mn)/rng;
    trend.push(`${x.toFixed(1)},${(mid-(norm-0.5)*half*1.5).toFixed(1)}`);
  });
  const conePath=`M ${top.join(" L ")} L ${[...bot].reverse().join(" L ")} Z`;
  const gid=`cone${w}x${h}`;
  return(
    <svg width={w} height={h} style={{display:"block",overflow:"visible"}}>
      <defs>
        <linearGradient id={gid} x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor={col} stopOpacity="0.08"/>
          <stop offset="55%" stopColor={col} stopOpacity="0.30"/>
          <stop offset="100%" stopColor={col} stopOpacity="0.55"/>
        </linearGradient>
      </defs>
      <path d={conePath} fill={`url(#${gid})`} stroke={col} strokeOpacity="0.65" strokeWidth="1.4" strokeLinejoin="round"
        className="breathe" style={{"--glow":`${col}80`}}/>
      <polyline points={trend.join(" ")} fill="none" stroke={col} strokeOpacity="0.9" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
      <circle cx={w-2} cy={mid} r="3.4" fill={col} className="pulse-slow" style={{color:col}}/>
    </svg>
  );
}
...
function PerfTable({title,rows,icon}){
  ...
  return(
    <div style={{background:"linear-gradient(135deg,rgba(255,255,255,0.045),rgba(255,255,255,0.012))",backdropFilter:"blur(18px)",WebkitBackdropFilter:"blur(18px)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:18,padding:"20px 22px",boxShadow:"0 8px 32px rgba(0,0,0,0.25)",transition:"transform .4s cubic-bezier(.4,0,.2,1),box-shadow .4s ease,border-color .4s ease",overflowX:"auto"}}>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0C0A08",color:"#fff",fontFamily:"'Outfit',sans-serif",direction:"rtl",position:"relative"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#170F08 0%,#2A1B08 40%,#170F08 100%)",borderBottom:"1px solid rgba(217,119,6,0.1)",padding:"24px 28px",position:"relative",overflow:"hidden",zIndex:1}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:24,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
            <EquityCone trades={c.trades} w={140} h={48}/>
          ...
          return(
            <button key={id} className={`tab-item${active?" active":""}`} onClick={()=>setTab(id)}
              style={{position:"relative",display:"flex",alignItems:"center",gap:10,padding:"9px 20px 11px",borderRadius:16,
                border:active?`1px solid ${color}4d`:"1px solid rgba(255,255,255,0.055)",
                background:active?`linear-gradient(135deg,${color}22,${color}08)`:"rgba(255,255,255,0.02)",
              ...
              <span className="tab-ico" style={{display:"flex",alignItems:"center",justifyContent:"center",width:26,height:26,borderRadius:9,flexShrink:0,
                background:active?`linear-gradient(135deg,${color}38,${color}12)`:"rgba(255,255,255,0.04)",
      ...
      {(()=>{
        ...
        if(!ins)return null;
        return(
          <div style={{padding:"13px 28px",background:`linear-gradient(135deg,${ins.color}14,${ins.color}03)`,borderBottom:`1px solid ${ins.color}22`,display:"flex",alignItems:"center",gap:11,position:"relative",zIndex:1}}>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0C0A08",color:"#fff",fontFamily:"'Outfit',sans-serif",direction:"rtl",position:"relative"}}>
      ...
      <div style={{padding:"16px 28px",background:"rgba(0,0,0,0.3)",borderBottom:"1px solid rgba(255,255,255,0.04)",display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {[
            ...
            <button key={k} className="btn-hover" onClick={()=>setSort(k)}
              style={{display:"flex",alignItems:"center",gap:6,background:sort===k?"linear-gradient(135deg,rgba(217,119,6,0.22),rgba(217,119,6,0.06))":"rgba(255,255,255,0.025)",color:sort===k?"#FCD34D":"rgba(255,255,255,0.4)",border:`1px solid ${sort===k?"rgba(217,119,6,0.35)":"rgba(255,255,255,0.06)"}`,borderRadius:10,padding:"8px 15px",fontSize:12,fontWeight:700,letterSpacing:".02em",fontFamily:"inherit",whiteSpace:"nowrap",backdropFilter:"blur(10px)"}}>
        ...
        <div style={{display:"flex",gap:4}}>
          {[
            ...
            <button key={v} className="btn-hover" onClick={()=>setView(v)} style={{display:"flex",alignItems:"center",justifyContent:"center",background:view===v?"linear-gradient(135deg,rgba(217,119,6,0.22),rgba(217,119,6,0.06))":"rgba(255,255,255,0.025)",color:view===v?"#FCD34D":"rgba(255,255,255,0.35)",border:`1px solid ${view===v?"rgba(217,119,6,0.35)":"rgba(255,255,255,0.06)"}`,borderRadius:10,padding:"8px 12px",fontFamily:"inherit",backdropFilter:"blur(10px)"}}>
              {ic}
            </button>
          ))}
          <button className="btn-hover" onClick={loadClients} style={{display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(255,255,255,0.025)",color:"rgba(255,255,255,0.4)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:"8px 12px",fontFamily:"inherit",backdropFilter:"blur(10px)"}}>
      ...
      <div style={{padding:"20px 28px 80px",position:"relative",zIndex:1}}>
            ...
            {filtered.map((c,idx)=>{
              ...
              return(
                <div key={c.uid} className="card-hover glow-pulse" onClick={()=>setSelected(c)}
                  style={{"--glow":`${sc}40`,background:"linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.012))",backdropFilter:"blur(18px)",WebkitBackdropFilter:"blur(18px)",border:`1px solid ${sc}22`,borderRadius:20,padding:"20px",overflow:"hidden",position:"relative",animation:`fadeUp .35s ease ${idx*0.04}s both`}}>
        ...
        {/* LIST */}
        {view==="list"&&(
          <div style={{background:"linear-gradient(135deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))",backdropFilter:"blur(18px)",WebkitBackdropFilter:"blur(18px)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:18,overflow:"hidden",boxShadow:"0 12px 40px rgba(0,0,0,0.3)"}}>
