      ...
      <div style={{padding:"24px 28px 60px",position:"relative",zIndex:1}}>
        ...
        {tab==="overview"&&(()=>{
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            {/* ── FULL METRICS TABLE (every computed stat, nothing hidden) ── */}
            <div style={{background:"#171310",border:"1px solid #3A2E1F",borderRadius:20,overflow:"hidden"}}>
              <div style={{padding:"14px 20px",borderBottom:"1px solid #3A2E1F",display:"flex",alignItems:"center",gap:8}}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#F59E0B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M3 10h18M9 4v18"/></svg>
                <p style={{fontSize:11,color:"#B8A98C",fontWeight:800,textTransform:"uppercase",letterSpacing:".08em"}}>كل المؤشرات — جدول كامل</p>
              </div>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}>
                  <tbody>
                    {[
                      ["إجمالي الصفقات",c.count],
                      ["صافي الربح/الخسارة","$"+(c.total?.toFixed(2)??0)],
                      ["نسبة الفوز",c.wr+"%"],
                      ["Profit Factor",c.profitFactor],
                      ["متوسط الربح/الصفقة","$"+(c.avgPnl?.toFixed(2)??0)],
                      ["Expectancy","$"+(c.expectancy?.toFixed(2)??0)],
                      ["متوسط RR",c.avgRR],
                      ["أكبر ربح","$"+(c.largestWin?.toFixed(2)??0)],
                      ["أكبر خسارة","$"+(c.largestLoss?.toFixed(2)??0)],
                      ["متوسط الربح (فوز)","$"+(c.avgWin?.toFixed(2)??0)],
                      ["متوسط الخسارة",  "$"+(c.avgLoss?.toFixed(2)??0)],
                      ["نسبة ربح/خسارة",c.winLossRatio],
                      ["إجمالي الربح","$"+(c.winTotal?.toFixed(2)??0)],
                      ["إجمالي الخسارة","$"+(c.lossTotal?.toFixed(2)??0)],
                      ["أقصى تراجع (Drawdown)","$"+(c.maxDrawdown?.toFixed(2)??0)+" ("+c.maxDrawdownPct+"%)"],
                      ["أطول سلسلة فوز",c.winStreak],
                      ["أطول سلسلة خسارة",c.lossStreak],
                      ["نسبة فوز — النصف الأول",c.wrFirst+"%"],
                      ["نسبة فوز — النصف الثاني",c.wrSecond+"%"],
                      ["صفقات بدون Stop Loss",c.noSLCount],
                      ["متوسط الشروط المحققة",c.avgConditions],
                      ["حجم التداول الكلي","$"+(c.totalVolume?.toFixed(2)??0)],
                      ["صفقات/أسبوع",c.tradesPerWeek],
                      ["صفقات شراء (Long)",c.longCount],
                      ["صفقات بيع (Short)",c.shortCount],
                      ["عدد الصور المرفقة",c.imageCount],
                      ["Score العام",c.score+"/100"],
                    ].map(([label,val],i)=>(
                      <tr key={i} style={{background:i%2===0?"#17130F":"#1D1610",borderBottom:"1px solid #241C13"}}>
                        <td style={{padding:"9px 20px",color:"#8A7B60",fontWeight:600}}>{label}</td>
                        <td style={{padding:"9px 20px",color:"#F1E9D8",fontWeight:800,fontFamily:"'JetBrains Mono',monospace",textAlign:"left"}}>{val ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
