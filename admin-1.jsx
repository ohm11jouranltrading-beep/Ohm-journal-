  ...
  return(
    <svg viewBox={`0 0 ${size} ${size}`} width="100%" height={size} style={{overflow:"visible"}}>
      {[20,40,60,80,100].map(lvl=>(
        <polygon key={lvl} points={axes.map((_,i)=>pt(i,r*lvl/100).join(",")).join(" ")} fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="1"/>
      ))}
      {axes.map((_,i)=>{const [x,y]=pt(i,r);return<line key={i} x1={cx} y1={cy} x2={x} y2={y} stroke="rgba(255,255,255,0.05)" strokeWidth="1"/>;})}
      <polygon points={polygonPts} fill="rgba(217,119,6,0.22)" stroke="#D97706" strokeWidth="2" strokeLinejoin="round"/>
      {dataPts.map((p,i)=><circle key={i} cx={p[0]} cy={p[1]} r="4" fill="#FCD34D" style={{filter:"drop-shadow(0 0 5px #FCD34D)"}}/>)}
      {axes.map((ax,i)=>{const [x,y]=pt(i,r+26);return<text key={i} x={x} y={y} fill="rgba(255,255,255,0.65)" fontSize="10.5" fontWeight="700" textAnchor="middle">{ax.label}</text>;})}
      {axes.map((ax,i)=>{const [x,y]=pt(i,r+26);return<text key={"v"+i} x={x} y={y+14} fill="#FCD34D" fontSize="10" fontWeight="800" textAnchor="middle">{Math.round(safeAxes[i].value)}%</text>;})}
    </svg>
  );
}
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0C0A08",color:"#fff",fontFamily:"'Outfit',sans-serif",direction:"rtl"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1.6fr 1fr",gap:16}}>
              <div style={{background:"linear-gradient(135deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))",border:"1px solid rgba(255,255,255,0.06)",borderRadius:18,padding:"20px 22px"}}>
                <p style={{fontSize:10,color:"rgba(255,255,255,0.35)",fontWeight:700,textTransform:"uppercase",letterSpacing:".1em",marginBottom:14}}>📈 Equity Overview</p>
                <EquityChart data={c.equityCurve} h={190}/>
              </div>
              <div style={{background:"linear-gradient(135deg,rgba(217,119,6,0.06),rgba(217,119,6,0.01))",border:"1px solid rgba(217,119,6,0.15)",borderRadius:18,padding:"18px 16px",display:"flex",flexDirection:"column",alignItems:"center"}}>
                <p style={{fontSize:10,color:"#FCD34D",fontWeight:700,textTransform:"uppercase",letterSpacing:".1em",marginBottom:6,alignSelf:"flex-start"}}>🧬 نقاط القوة والضعف</p>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              <div style={{background:"linear-gradient(135deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))",border:"1px solid rgba(255,255,255,0.06)",borderRadius:18,padding:"20px 22px",display:"flex",gap:18,alignItems:"center"}}>
              ...
              <div style={{background:"linear-gradient(135deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))",border:"1px solid rgba(255,255,255,0.06)",borderRadius:18,padding:"20px 22px"}}>
                <p style={{fontSize:10,color:"rgba(255,255,255,0.35)",fontWeight:700,textTransform:"uppercase",letterSpacing:".1em",marginBottom:14}}>🔻 قمع جودة التنفيذ (Execution Funnel)</p>
                <Funnel steps={[
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              <div style={{background:"linear-gradient(135deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))",border:"1px solid rgba(255,255,255,0.06)",borderRadius:18,padding:"20px 22px"}}>
                <p style={{fontSize:10,color:"rgba(255,255,255,0.35)",fontWeight:700,textTransform:"uppercase",letterSpacing:".1em",marginBottom:14}}>🎯 أفضل الإعدادات أداءً</p>
                <BreakdownBarChart rows={c.setupStats} valueKey="pnl"/>
              </div>
              <div style={{background:"linear-gradient(135deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))",border:"1px solid rgba(255,255,255,0.06)",borderRadius:18,padding:"20px 22px"}}>
                <p style={{fontSize:10,color:"rgba(255,255,255,0.35)",fontWeight:700,textTransform:"uppercase",letterSpacing:".1em",marginBottom:14}}>🕒 آخر النشاطات</p>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      {kpiSel&&(()=>{
        ...
        return(
          <div onClick={()=>setKpiSel(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",backdropFilter:"blur(8px)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
            <div onClick={e=>e.stopPropagation()} style={{background:"#171310",border:`1px solid ${kpiSel.color}30`,borderRadius:24,width:"100%",maxWidth:620,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 40px 100px rgba(0,0,0,0.8)"}}>
              {/* header */}
              <div style={{background:`linear-gradient(135deg,${kpiSel.color}18,rgba(217,119,6,0.04))`,borderBottom:"1px solid rgba(255,255,255,0.06)",padding:"22px 24px",borderRadius:"24px 24px 0 0",display:"flex",alignItems:"center",gap:14}}>
                <div style={{fontSize:30}}>{kpiSel.icon}</div>
                <div style={{flex:1}}>
                  <p style={{fontSize:11,color:"rgba(255,255,255,0.4)",fontWeight:700,textTransform:"uppercase",letterSpacing:".08em"}}>{kpiSel.label}</p>
                  <p style={{fontSize:28,fontWeight:900,color:kpiSel.color,fontFamily:"'JetBrains Mono',monospace"}}>{kpiSel.value}</p>
                </div>
                <button onClick={()=>setKpiSel(null)} style={{background:"rgba(255,255,255,0.06)",border:"none",borderRadius:8,width:32,height:32,color:"#fff",cursor:"pointer",fontSize:16}}>✕</button>
              </div>

              <div style={{padding:"22px 24px",display:"flex",flexDirection:"column",gap:18}}>
                {/* explanation */}
                <div>
                  <p style={{fontSize:10,color:"rgba(255,255,255,0.35)",fontWeight:700,textTransform:"uppercase",letterSpacing:".08em",marginBottom:6}}>📖 ما معنى هذا الرقم؟</p>
                  <p style={{fontSize:13,color:"rgba(255,255,255,0.7)",lineHeight:1.8}}>{info.explain}</p>
                </div>

                {/* verdict / guidance */}
                <div style={{background:`${info.verdict.c}14`,border:`1px solid ${info.verdict.c}35`,borderRadius:12,padding:"14px 16px",display:"flex",gap:10,alignItems:"flex-start"}}>
                  <span style={{fontSize:16,flexShrink:0}}>💡</span>
                  <p style={{fontSize:13,color:"#fff",fontWeight:600,lineHeight:1.7}}>{info.verdict.t}</p>
                </div>

                {/* dedicated chart */}
                <div>
                  <p style={{fontSize:10,color:"rgba(255,255,255,0.35)",fontWeight:700,textTransform:"uppercase",letterSpacing:".08em",marginBottom:10}}>📊 السياق البياني</p>
                  ...
                  {info.chart==="rmultiple"&&(
                    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                      {[{lbl:"3R+",n:c.rMultiples?.r3,col:"#10b981"},{lbl:"2R–3R",n:c.rMultiples?.r2,col:"#34d399"},{lbl:"1R–2R",n:c.rMultiples?.r1,col:"#FCD34D"},{lbl:"خسارة",n:c.rMultiples?.rNeg,col:"#ef4444"}].map(x=>(
                        <div key={x.lbl} style={{textAlign:"center",background:"rgba(255,255,255,0.03)",borderRadius:10,padding:"12px 6px"}}>
