...
const CSS=`
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ── MATTE DARK BASE (opaque, textured — not see-through glass) ── */
  .glass{
    background:#161006;
    background-image:radial-gradient(circle at 20% 15%,rgba(255,255,255,0.035),transparent 55%);
  ...
  .glass-gold{
    background:linear-gradient(135deg,#221A12,#160e04);
    backdrop-filter:blur(6px);
    -webkit-backdrop-filter:blur(6px);
  ...
  /* ── ROW HOVER ── */
  .row-hover{transition:background .2s ease, transform .2s ease;}
  .row-hover:hover{background:rgba(217,119,6,0.07)!important;transform:translateX(-3px);}
  .row-hover:hover .avatar-glow{box-shadow:0 0 16px var(--glow,rgba(217,119,6,0.4));transform:scale(1.08);}
  ...
  .modal-glass{
    ...
    border-radius:24px;
    box-shadow:0 60px 120px rgba(0,0,0,0.85),0 0 0 1px rgba(255,255,255,0.03) inset;
  }

  /* ══════════════════════════════════════════════ */
  /*  UNIVERSAL INTERACTIVE GLASS SYSTEM             */
  /* ══════════════════════════════════════════════ */

  /* ── CARD HOVER (client cards, KPI tiles, trade thumbnails...) ── */
  .card-hover{
    position:relative;cursor:pointer;overflow:hidden;isolation:isolate;
  ...
  .card-hover:hover{
    ...
    box-shadow:0 22px 54px rgba(0,0,0,0.45),0 0 26px rgba(217,119,6,0.16);
    border-color:rgba(217,119,6,0.35)!important;
  }
  .card-hover:hover::before{left:135%;}
  .card-hover:active{transform:translateY(-1px) scale(0.985);}

  /* ── BUTTON HOVER (all action buttons) ── */
  .btn-hover{
    cursor:pointer;position:relative;overflow:hidden;
    transition:transform .22s cubic-bezier(.34,1.56,.64,1), filter .22s ease, box-shadow .22s ease;
  }
  .btn-hover::after{
    content:'';position:absolute;inset:0;border-radius:inherit;
  ...
  .btn-hover:hover{transform:translateY(-2px);filter:brightness(1.18);box-shadow:0 10px 26px rgba(217,119,6,0.22);}
  .btn-hover:hover::after{opacity:1;}
  .btn-hover:active{transform:translateY(0) scale(0.95);}

  /* ── PROFILE TABS (glass pills w/ glowing icons) ── */
  .tab-item{
    position:relative;cursor:pointer;overflow:hidden;flex-shrink:0;
    transition:transform .28s cubic-bezier(.34,1.56,.64,1), background .28s ease, border-color .28s ease, box-shadow .28s ease;
  }
  .tab-item::before{
  ...
  .tab-item:hover::before{left:135%;}
  .tab-item.active{animation:tabGlow 2.4s ease-in-out infinite;}
  .tab-item .tab-ico{transition:filter .28s ease,transform .28s ease,color .28s ease;}
  .tab-item:hover .tab-ico{transform:scale(1.12);}
  @keyframes tabGlow{0%,100%{box-shadow:0 4px 16px rgba(217,119,6,0.14)}50%{box-shadow:0 4px 24px rgba(217,119,6,0.32)}}

  /* ── PULSING SCORE-COLORED GLOW (client list cards) ── */
  .glow-pulse{animation:glowPulseVar 3.2s ease-in-out infinite;}
  @keyframes glowPulseVar{
    0%,100%{box-shadow:0 4px 24px rgba(0,0,0,0.3),0 0 0 1px var(--glow,rgba(217,119,6,0.15)),0 0 0px var(--glow,transparent)}
    50%{box-shadow:0 4px 24px rgba(0,0,0,0.3),0 0 0 1px var(--glow,rgba(217,119,6,0.25)),0 0 22px var(--glow,transparent)}
  }
  .avatar-glow{transition:box-shadow .3s ease,transform .3s ease;}
  .card-hover:hover .avatar-glow{box-shadow:0 0 20px var(--glow,rgba(217,119,6,0.4));transform:scale(1.06);}

  /* ══════════════════════════════════════════════ */
  /*  LIVING PLATFORM — slow ambient life            */
  /* ══════════════════════════════════════════════ */

  /* ── starfield: slow drifting + twinkling dots ── */
  .starfield{position:fixed;inset:0;overflow:hidden;pointer-events:none;z-index:0;}
  .star{position:absolute;border-radius:50%;background:#fff;opacity:.55;box-shadow:0 0 4px rgba(255,255,255,0.6);}
  @keyframes starDrift{0%{transform:translate(0,0)}100%{transform:translate(-36px,26px)}}
  @keyframes starTwinkle{0%,100%{opacity:.15}50%{opacity:.9}}

  /* ── slow breathing pulse for important icons/badges ── */
  .pulse-slow{display:inline-flex;animation:pulseSlow 3.4s ease-in-out infinite;transform-origin:center;}
  @keyframes pulseSlow{0%,100%{transform:scale(1);filter:drop-shadow(0 0 0px currentColor)}50%{transform:scale(1.14);filter:drop-shadow(0 0 8px currentColor)}}

  /* ── slow breathing glow for containers (score arc, cone chart, key cards) ── */
  .breathe{animation:breatheGlow 3.8s ease-in-out infinite;}
  @keyframes breatheGlow{0%,100%{filter:drop-shadow(0 0 2px var(--glow,rgba(217,119,6,.3)))}50%{filter:drop-shadow(0 0 10px var(--glow,rgba(217,119,6,.6)))}}
  .breathe-box{animation:breatheBox 3.8s ease-in-out infinite;}
  @keyframes breatheBox{0%,100%{box-shadow:0 0 8px var(--glow,rgba(217,119,6,.25))}50%{box-shadow:0 0 22px var(--glow,rgba(217,119,6,.55))}}
`;
...
/* ── Equity Cone (funnel-shaped growth visual, replaces plain sparkline at header) ── */
function EquityCone({trades,w=64,h=64}){
  if(!trades?.length)return<div style={{width:w,height:h,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:10,color:"rgba(255,255,255,0.15)"}}>—</span></div>;
  const wins=trades.filter(t=>(t.pnl||0)>0).length;
  const losses=trades.filter(t=>(t.pnl||0)<0).length;
  const flat=trades.length-wins-losses;
  const size=Math.min(w,h);
  const total=wins+losses+flat||1;
  const r=size/2-8,cx=size/2,cy=size/2;
  let cum=-90;
  const toXY=ang=>{const rad=ang*Math.PI/180;return[cx+r*Math.cos(rad),cy+r*Math.sin(rad)];};
  const segs=[{v:wins,c:"#10b981"},{v:losses,c:"#ef4444"},{v:flat,c:"rgba(255,255,255,0.15)"}].filter(s=>s.v>0);
  const arcs=segs.map(s=>{
    const pct=s.v/total,start=cum,end=cum+pct*360;cum=end;
    const large=end-start>180?1:0;
    const [x1,y1]=toXY(start),[x2,y2]=toXY(end);
    const full=pct>=0.999;
    const d=full?`M ${cx-r},${cy} A ${r},${r} 0 1 1 ${cx+r},${cy} A ${r},${r} 0 1 1 ${cx-r},${cy} Z`
                :`M ${cx},${cy} L ${x1},${y1} A ${r},${r} 0 ${large} 1 ${x2},${y2} Z`;
    return {d,color:s.c};
  });
  const wr=Math.round(wins/(wins+losses||1)*100);
  const wrCol=wr>=55?"#10b981":wr>=40?"#f59e0b":"#ef4444";
  return(
    <div style={{position:"relative",width:size,height:size}}>
      <svg width={size} height={size}>
        {arcs.map((a,i)=><path key={i} d={a.d} fill={a.color} stroke="#171310" strokeWidth="2.5"/>)}
        <circle cx={cx} cy={cy} r={r*0.56} fill="#171310"/>
      </svg>
      <div className="breathe" style={{"--glow":`${wrCol}80`,position:"absolute",inset:0,borderRadius:"50%"}}/>
      <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
        <span style={{fontSize:size*0.24,fontWeight:900,color:wrCol,fontFamily:"'JetBrains Mono',monospace",lineHeight:1}}>{wr}%</span>
        <span style={{fontSize:size*0.09,fontWeight:700,color:"rgba(255,255,255,0.3)",letterSpacing:".05em"}}>WIN</span>
      </div>
    </div>
  );
}

/* ── Starfield (slow drifting + twinkling background) ── */
function Starfield({count=50}){
  const stars=useMemo(()=>Array.from({length:count},(_,i)=>({
    ...
    dur:Math.random()*50+35,
    delay:-(Math.random()*40),
    tDur:Math.random()*3.5+2.5,
    tDelay:-(Math.random()*5),
  })),[count]);
  return(
    <div className="starfield" aria-hidden="true">
      {stars.map(s=>(
        <div key={s.id} className="star" style={{
          ...
          animation:`starDrift ${s.dur}s linear ${s.delay}s infinite alternate, starTwinkle ${s.tDur}s ease-in-out ${s.tDelay}s infinite`,
        }}/>
      ))}
    </div>
  );
...
function PerfTable({title,rows,icon}){
  ...
  return(
    <div style={{background:"#171310",backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",border:"1px solid #3A2E1F",borderRadius:18,padding:"20px 22px",boxShadow:"0 8px 32px rgba(0,0,0,0.25)",transition:"transform .4s cubic-bezier(.4,0,.2,1),box-shadow .4s ease,border-color .4s ease",overflowX:"auto"}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",minWidth:420}}>
        <thead>
          <tr style={{background:"#17130F"}}>
            {["البند","عدد","نسبة الفوز","متوسط RR","صافي الربح"].map((h,i)=>(
              <th key={i} style={{fontSize:9,color:"#8A7B60",fontWeight:700,textTransform:"uppercase",letterSpacing:".06em",padding:"9px 8px",textAlign:i===0?"right":"center",borderBottom:"1px solid #3A2E1F"}}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((r,i)=>(
            <tr key={r.key} className="row-hover" style={{background:i%2===0?"#17130F":"#1D1610",borderBottom:"1px solid #241C13"}}>
              <td style={{padding:"9px 8px",fontSize:13,fontWeight:700,color:"#F1E9D8"}}>{r.key}</td>
              <td style={{padding:"9px 8px",fontSize:12,textAlign:"center",color:"#B8A98C",fontFamily:"'JetBrains Mono',monospace"}}>{r.count}</td>
...
/* ══════════════════════════════════════════════ */
/*  DASHBOARD PRIMITIVES — sectioned metrics view  */
/* ══════════════════════════════════════════════ */

/* ── Big Number Card (Overview KPIs) ── */
function BigNumberCard({icon,label,value,color="#F59E0B",arrow,progress}){
  return(
    <div className="card-hover breathe-box" style={{"--glow":`${color}30`,background:"#171310",border:`1px solid ${color}30`,borderRadius:18,padding:"18px 20px",position:"relative",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
        <span style={{fontSize:9,color:"#8A7B60",fontWeight:800,textTransform:"uppercase",letterSpacing:".08em"}}>{label}</span>
        <span style={{fontSize:15}}>{icon}</span>
      </div>
      <div style={{display:"flex",alignItems:"baseline",gap:6}}>
        <p style={{fontSize:30,fontWeight:900,color,fontFamily:"'JetBrains Mono',monospace",lineHeight:1}}>{value}</p>
        {arrow&&<span style={{fontSize:16,color}}>{arrow>0?"↑":arrow<0?"↓":""}</span>}
      </div>
      {progress!=null&&(
        <div style={{height:6,background:"#0C0A08",borderRadius:4,overflow:"hidden",marginTop:12}}>
          <div className="progress-fill" style={{"--target-w":`${Math.min(100,Math.max(0,progress))}%`,height:"100%",width:`${Math.min(100,Math.max(0,progress))}%`,background:`linear-gradient(90deg,${color}80,${color})`,borderRadius:4}}/>
        </div>
      )}
    </div>
  );
...
function RadialGauge({value,max=100,label,color="#F59E0B",unit=""}){
  const pct=Math.max(0,Math.min(1,value/max));
  const size=140,r=56,cx=size/2,cy=size/2+8;
  const a0=180,a1=180-pct*180;
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
      <svg width={size} height={size*0.62} viewBox={`0 0 ${size} ${size*0.62}`}>
        <path d={`M ${x0},${y0} A ${r},${r} 0 0 1 ${trackEnd[0]},${trackEnd[1]}`} stroke="#241C13" strokeWidth="12" fill="none" strokeLinecap="round"/>
        <path d={`M ${x0},${y0} A ${r},${r} 0 0 1 ${x1},${y1}`} stroke={color} strokeWidth="12" fill="none" strokeLinecap="round" className="breathe" style={{"--glow":`${color}80`}}/>
      </svg>
      <p style={{fontSize:22,fontWeight:900,color,fontFamily:"'JetBrains Mono',monospace",marginTop:-38}}>{value}{unit}</p>
      <p style={{fontSize:9,color:"#8A7B60",fontWeight:700,textTransform:"uppercase",letterSpacing:".08em",marginTop:16}}>{label}</p>
    </div>
  );
}

/* ── Dual (grouped) Bar Comparison ── */
function DualBar({title,aLabel,aVal,aColor,bLabel,bVal,bColor,fmt=v=>"$"+v.toFixed(0)}){
  const max=Math.max(Math.abs(aVal),Math.abs(bVal),1);
  return(
    <div style={{background:"#171310",border:"1px solid #3A2E1F",borderRadius:16,padding:"16px 18px"}}>
      <p style={{fontSize:9,color:"#8A7B60",fontWeight:800,textTransform:"uppercase",letterSpacing:".08em",marginBottom:14}}>{title}</p>
      <div style={{display:"flex",alignItems:"flex-end",gap:18,height:100,justifyContent:"center"}}>
        {[{l:aLabel,v:aVal,c:aColor},{l:bLabel,v:bVal,c:bColor}].map((b,i)=>(
          <div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,width:64}}>
            <span style={{fontSize:12,fontWeight:800,color:b.c,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(b.v)}</span>
            <div className="breathe-box" style={{"--glow":`${b.c}50`,width:"100%",height:Math.max(6,(Math.abs(b.v)/max)*72),background:`linear-gradient(180deg,${b.c},${b.c}70)`,borderRadius:8}}/>
            <span style={{fontSize:9,color:"#8A7B60",fontWeight:700,textAlign:"center"}}>{b.l}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:5}}>
        <span style={{color:"#8A7B60",fontWeight:700}}>{label}</span>
        <span style={{color,fontWeight:800,fontFamily:"'JetBrains Mono',monospace"}}>{value}%</span>
      </div>
      <div style={{height:10,background:"#0C0A08",borderRadius:5,overflow:"hidden"}}>
        <div className="progress-fill" style={{"--target-w":`${value}%`,height:"100%",width:`${value}%`,background:value>0?`linear-gradient(90deg,${color}70,${color})`:"transparent",borderRadius:5}}/>
      </div>
    </div>
  );
}

/* ── Segmentation Bar (Long vs Short etc.) ── */
function SegmentBar({segments,title}){
  const total=segments.reduce((s,x)=>s+x.value,0)||1;
  return(
    <div style={{background:"#171310",border:"1px solid #3A2E1F",borderRadius:16,padding:"16px 18px"}}>
      <p style={{fontSize:9,color:"#8A7B60",fontWeight:800,textTransform:"uppercase",letterSpacing:".08em",marginBottom:12}}>{title}</p>
      <div style={{display:"flex",height:16,borderRadius:8,overflow:"hidden",marginBottom:10}}>
        {segments.map((s,i)=>(
          <div key={i} style={{width:`${Math.max(2,(s.value/total)*100)}%`,background:s.color}}/>
        ))}
      </div>
      <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
        {segments.map((s,i)=>(
          <span key={i} style={{fontSize:11,color:"#B8A98C",display:"flex",alignItems:"center",gap:6}}>
            <span style={{width:8,height:8,borderRadius:"50%",background:s.color,display:"inline-block"}}/>
            {s.label} <b style={{color:"#F1E9D8",fontFamily:"'JetBrains Mono',monospace"}}>{s.value}</b>
          </span>
        ))}
      </div>
    </div>
...
/* ── Drawdown Area Chart (flat baseline, red valleys below) ── */
function DrawdownArea({equityCurve,h=120}){
  if(!equityCurve?.length)return<div style={{height:h}}/>;
  let peak=0;
  const dd=equityCurve.map(p=>{peak=Math.max(peak,p.cum);return peak-p.cum;});
  const max=Math.max(...dd,1);
  const w=100,n=dd.length;
  const pts=dd.map((v,i)=>`${(i/(n-1||1))*w},${(v/max)*h}`);
  const areaPath=`M 0,0 L ${pts.join(" L ")} L ${w},0 Z`;
  return(
    <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={h} preserveAspectRatio="none">
      <line x1="0" y1="0" x2={w} y2="0" stroke="#3A2E1F" strokeWidth="1"/>
      <path d={areaPath} fill="rgba(239,68,68,0.18)" stroke="#ef4444" strokeWidth="1" vectorEffect="non-scaling-stroke"/>
    </svg>
  );
}

/* ── Streak Counter (trophy / X) ── */
function StreakCounter({icon,label,value,color}){
  return(
    <div className="card-hover" style={{background:"#171310",border:`1px solid ${color}30`,borderRadius:16,padding:"18px",display:"flex",alignItems:"center",gap:14}}>
      <div className="pulse-slow" style={{width:44,height:44,borderRadius:12,background:`${color}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>{icon}</div>
      <div>
        <p style={{fontSize:24,fontWeight:900,color,fontFamily:"'JetBrains Mono',monospace",lineHeight:1}}>{value}</p>
        <p style={{fontSize:10,color:"#8A7B60",fontWeight:700,marginTop:2}}>{label}</p>
      </div>
    </div>
  );
}

/* ── Small Metric Chip (grouped detail cards) ── */
function MetricChip({icon,label,value,color="#F59E0B"}){
  return(
    <div className="card-hover" style={{background:"#171310",border:"1px solid #3A2E1F",borderRadius:14,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
      <div style={{width:34,height:34,borderRadius:10,background:`${color}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,flexShrink:0}}>{icon}</div>
      <div style={{minWidth:0}}>
        <p style={{fontSize:16,fontWeight:900,color:"#F1E9D8",fontFamily:"'JetBrains Mono',monospace",lineHeight:1.1}}>{value}</p>
        <p style={{fontSize:9,color:"#8A7B60",fontWeight:700,marginTop:3,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{label}</p>
      </div>
    </div>
  );
}
...
function SectionHead({icon,title}){
  return(
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
      ...
      <p style={{fontSize:11,color:"#B8A98C",fontWeight:800,textTransform:"uppercase",letterSpacing:".08em"}}>{title}</p>
      <div style={{flex:1,height:1,background:"#241C13"}}/>
    </div>
  );
}

/* ── Profit Trend Arrow (visual arrow tilts with the overall trend) ── */
function ProfitArrowChart({equityCurve}){
  const n=equityCurve?.length||0;
  const first=equityCurve?.[0]?.cum||0, last=equityCurve?.[n-1]?.cum||0;
  const changePct=first!==0?((last-first)/Math.abs(first))*100:(last>0?100:last<0?-100:0);
  const up=last>=first;
  const col=up?"#10b981":"#ef4444";
  const w=100,h=70;
  const ys=equityCurve?.map(p=>p.cum)||[0];
  const mn=Math.min(...ys,0),mx=Math.max(...ys,0),rng=(mx-mn)||1;
  const pts=(equityCurve||[]).map((p,i)=>`${(i/((n-1)||1))*w},${h-((p.cum-mn)/rng)*h}`);
  const angle=up?-Math.min(35,Math.abs(changePct)/3):Math.min(35,Math.abs(changePct)/3);
  return(
    <div className="breathe-box" style={{"--glow":`${col}35`,background:"#171310",border:`1px solid ${col}30`,borderRadius:18,padding:"18px 20px",position:"relative",overflow:"hidden"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        <span style={{fontSize:11,color:"#8A7B60",fontWeight:700}}>سهم اتجاه الأرباح</span>
        <span style={{fontSize:13,fontWeight:900,color:col,fontFamily:"'JetBrains Mono',monospace"}}>{changePct>=0?"+":""}{changePct.toFixed(0)}%</span>
      </div>
      <div style={{position:"relative",height:h,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={h} preserveAspectRatio="none" style={{position:"absolute",inset:0}}>
          {pts.length>1&&<polyline points={pts.join(" ")} fill="none" stroke={col} strokeOpacity="0.35" strokeWidth="2" vectorEffect="non-scaling-stroke"/>}
        </svg>
        <svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke={col} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"
          className="pulse-slow" style={{transform:`rotate(${angle}deg)`,filter:`drop-shadow(0 0 8px ${col}aa)`}}>
          <line x1="3" y1="12" x2="21" y2="12"/><polyline points="14 5 21 12 14 19"/>
        </svg>
      </div>
      <p style={{fontSize:10,color:"#8A7B60",marginTop:6,textAlign:"center"}}>{up?"الاتجاه العام تصاعدي — استمرارية جيدة":"الاتجاه العام تنازلي — يحتاج تصحيح فوري"}</p>
    </div>
  );
}

/* ── Profit Projection (forecast based on current pace) ── */
function ProfitProjectionChart({c}){
  const perWeek=(c.expectancy||0)*(Number(c.tradesPerWeek)||0);
  const cur=c.total||0;
  const proj1w=cur+perWeek, proj1m=cur+perWeek*4, proj3m=cur+perWeek*12;
  const hist=c.equityCurve||[];
  const n=hist.length;
  const allYs=[...hist.map(p=>p.cum),proj1w,proj1m,proj3m,0];
  const mn=Math.min(...allYs),mx=Math.max(...allYs),rng=(mx-mn)||1;
  const w=100,h=90;
  const stepX=w/((n+2)||1);
  const histPts=hist.map((p,i)=>`${(i*stepX).toFixed(2)},${(h-((p.cum-mn)/rng)*h).toFixed(2)}`);
  const lastX=Math.max(0,(n-1))*stepX;
  const projPts=[
    [lastX,h-((cur-mn)/rng)*h],
    [lastX+stepX,h-((proj1w-mn)/rng)*h],
    [lastX+stepX*1.6,h-((proj1m-mn)/rng)*h],
    [lastX+stepX*2.4,h-((proj3m-mn)/rng)*h],
  ];
  const col=perWeek>=0?"#10b981":"#ef4444";
  return(
    <div style={{background:"#171310",border:`1px solid ${col}30`,borderRadius:18,padding:"18px 20px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:6}}>
        <span style={{fontSize:11,color:"#8A7B60",fontWeight:700}}>توقع الأرباح بناءً على الأداء الحالي</span>
        <span style={{fontSize:10,color:col,fontWeight:800,fontFamily:"'JetBrains Mono',monospace"}}>≈{perWeek>=0?"+":""}${perWeek.toFixed(0)}/أسبوع</span>
      </div>
      <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={h} preserveAspectRatio="none">
        <line x1="0" y1={h-((0-mn)/rng)*h} x2={w} y2={h-((0-mn)/rng)*h} stroke="#3A2E1F" strokeWidth="1" vectorEffect="non-scaling-stroke"/>
        {histPts.length>1&&<polyline points={histPts.join(" ")} fill="none" stroke="#FCD34D" strokeWidth="2" vectorEffect="non-scaling-stroke"/>}
        <polyline points={projPts.map(p=>p.join(",")).join(" ")} fill="none" stroke={col} strokeWidth="2" strokeDasharray="4,3" vectorEffect="non-scaling-stroke"/>
        {projPts.map(([x,y],i)=><circle key={i} cx={x} cy={y} r="2.2" fill={col} className={i===projPts.length-1?"pulse-slow":""}/>)}
      </svg>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginTop:14}}>
        {[["أسبوع",proj1w],["شهر",proj1m],["3 أشهر",proj3m]].map(([lbl,val],i)=>(
          <div key={i} style={{textAlign:"center"}}>
            <p style={{fontSize:9,color:"#8A7B60",fontWeight:700,marginBottom:3}}>بعد {lbl}</p>
            <p style={{fontSize:14,fontWeight:900,color:val>=0?"#10b981":"#ef4444",fontFamily:"'JetBrains Mono',monospace"}}>{val>=0?"+":""}${val.toFixed(0)}</p>
          </div>
        ))}
      </div>
      <p style={{fontSize:9,color:"rgba(255,255,255,0.25)",marginTop:10,textAlign:"center"}}>* تقدير تقريبي مبني على متوسط الأداء الحالي، وليس ضمانًا لنتائج مستقبلية.</p>
    </div>
  );
}
...
function MiniStatCard({icon,label,value,color,trend,trades,onClick}){
  return(
    <div onClick={onClick} className="card-hover" style={{cursor:onClick?"pointer":"default",background:"linear-gradient(160deg,rgba(255,255,255,0.035),rgba(255,255,255,0.008))",border:"1px solid #3A2E1F",borderRadius:16,padding:"16px 18px",position:"relative",overflow:"hidden"}}>
...
function TradeModal({trade,onClose}){
  ...
  const exitVal=pick(trade.exit,trade.exitPrice,trade.exit_price,trade.closePrice,trade.close,trade.exitP);
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",backdropFilter:"blur(8px)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div onClick={e=>e.stopPropagation()} className="modal-glass" style={{width:"100%",maxWidth:540,maxHeight:"90vh",overflowY:"auto",position:"relative",animation:"popIn .35s cubic-bezier(.34,1.56,.64,1)"}}>
        {/* modal header */}
        <div style={{background:`linear-gradient(135deg,${isW?"rgba(16,185,129,0.1)":"rgba(239,68,68,0.1)"},rgba(217,119,6,0.05))`,borderBottom:"1px solid #3A2E1F",padding:"22px 24px",borderRadius:"24px 24px 0 0"}}>
        ...
        <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
            {[
              {l:"Entry Price",v:pick(trade.entry),c:"#D97706",ic:"🎯"},
              {l:"Exit Price",v:exitVal,c:isW?"#10b981":"#ef4444",ic:isW?"🏁":"🔚"},
              {l:"Lot Size",v:pick(trade.lotSize),c:"#f59e0b",ic:"📦"},
            ].map(({l,v,c,ic})=>(
              <div key={l} className="stat-glass" style={{padding:"12px 14px"}}>
                <p style={{fontSize:9,color:"rgba(255,255,255,0.35)",fontWeight:700,textTransform:"uppercase",letterSpacing:".08em",marginBottom:4}}>{ic} {l}</p>
          ...
          {/* Notes */}
          {trade.notes&&(
            <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid #3A2E1F",borderRadius:12,padding:"14px 16px"}}>
          ...
          {/* Chart Image */}
          {trade.image&&(
            <div style={{borderRadius:12,overflow:"hidden",border:"1px solid #3A2E1F"}}>
...
function ProfilePage({client:c,onBack}){
  ...
  const sorted=[...(Array.isArray(c.trades)?c.trades:[])].sort((a,b)=>(b.date||"").localeCompare(a.date||""));
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0C0A08",color:"#fff",fontFamily:"'Outfit',sans-serif",direction:"rtl",position:"relative"}}>
      <style>{CSS}</style>
      <Starfield count={60}/>

      {/* HERO HEADER */}
      <div style={{background:"linear-gradient(135deg,#170F08 0%,#2A1B08 40%,#170F08 100%)",borderBottom:"1px solid rgba(217,119,6,0.1)",padding:"24px 28px",position:"relative",overflow:"hidden",zIndex:1}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:24,flexWrap:"wrap"}}>
          ...
          <div style={{position:"relative"}}>
            ...
            <div className="pulse-slow" style={{position:"absolute",bottom:-4,right:-4,width:20,height:20,borderRadius:"50%",background:scoreCol,border:"2px solid #0C0A08",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:"#0C0A08"}}>✓</div>
          </div>

          <div style={{flex:1}}>
            <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",marginBottom:6}}>
              <h1 style={{fontSize:30,fontWeight:900,color:"#fff",letterSpacing:"-.03em"}}>{c.name}</h1>
              <span className={c.score>=70?"pulse-slow":""} style={{background:`${scoreCol}15`,color:scoreCol,border:`1px solid ${scoreCol}30`,borderRadius:20,padding:"4px 14px",fontSize:11,fontWeight:700,letterSpacing:".06em"}}>
          ...
          <div className="breathe" style={{"--glow":`${scoreCol}70`,display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            <EquityCone trades={c.trades} w={72} h={72}/>
            <p style={{fontSize:9,color:"rgba(255,255,255,0.2)",letterSpacing:".06em",textTransform:"uppercase"}}>Win / Loss Split</p>
          </div>
        </div>
      </div>
      ...
      {c.riskFlags?.length>0&&(
        <div style={{padding:"16px 28px",background:"rgba(0,0,0,0.35)",borderBottom:"1px solid rgba(255,255,255,0.05)",position:"relative",zIndex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            <span className="pulse-slow" style={{fontSize:14}}>🛡️</span>
      ...
      {/* TABS */}
      <div style={{display:"flex",gap:10,background:"rgba(0,0,0,0.35)",borderBottom:"1px solid rgba(255,255,255,0.05)",padding:"14px 28px",overflowX:"auto",position:"relative",zIndex:1}}>
        {[
          {id:"overview",label:"Overview",color:"#F59E0B",icon:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/></svg>},
          {id:"trades",label:`Trades (${c.count})`,color:"#FCD34D",icon:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/><line x1="9" y1="7" x2="15" y2="7"/><line x1="9" y1="11" x2="15" y2="11"/></svg>},
          {id:"analysis",label:"Analysis",color:"#FB923C",icon:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>},
          {id:"psychology",label:"Psychology",color:"#D97706",icon:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a7 7 0 0 1 7 7c0 2.5-1.3 4.7-3.3 6L15 21H9l-.3-5.8A7 7 0 0 1 12 2z"/><line x1="9" y1="21" x2="15" y2="21"/></svg>},
        ].map(({id,label,color,icon})=>{
          const active=tab===id;
          return(
            <button key={id} className={`tab-item${active?" active":""}`} onClick={()=>setTab(id)}
              style={{position:"relative",display:"flex",alignItems:"center",gap:10,padding:"9px 20px 11px",borderRadius:16,
                border:active?`1px solid ${color}55`:"1px solid #3A2E1F",
                background:active?`linear-gradient(135deg,${color}2e,#120d07)`:"#120d07",
                backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",
                fontSize:13,fontWeight:800,letterSpacing:".01em",color:active?color:"rgba(255,255,255,0.42)",whiteSpace:"nowrap",fontFamily:"'Outfit',sans-serif"}}>
              <span className="tab-ico" style={{display:"flex",alignItems:"center",justifyContent:"center",width:26,height:26,borderRadius:9,flexShrink:0,
                background:active?`linear-gradient(135deg,${color}45,${color}15)`:"rgba(255,255,255,0.05)",
                color:active?color:"rgba(255,255,255,0.35)",filter:active?`drop-shadow(0 0 5px ${color}bb)`:"none"}}>{icon}</span>
              {label}
              {active&&<span style={{position:"absolute",bottom:0,left:"22%",right:"22%",height:2,borderRadius:2,background:color,boxShadow:`0 0 8px ${color}`}}/>}
            </button>
          );
        })}
      </div>

      {/* ── STATUS INSIGHT (explains what the active tab's data means) ── */}
      {(()=>{
        const wr=c.wr,score=c.score,pf=parseFloat(c.profitFactor)||0,critical=c.riskFlags?.filter(f=>f.level==="high").length||0;
        const M={
          overview: score>=70?{icon:"📈",color:"#10b981",text:`أداء ممتاز — Score ${score}/100 والاتجاه العام تصاعدي. استمر بنفس مستوى الانضباط.`}
                  : score>=45?{icon:"⚖️",color:"#f59e0b",text:`أداء متوسط — Score ${score}/100. فيه مجال حقيقي للتحسين، راجع نقاط الضعف بالأسفل.`}
                  : {icon:"🚨",color:"#ef4444",text:`أداء ضعيف — Score ${score}/100. يحتاج مراجعة شاملة للخطة والانضباط فورًا.`},
          trades: wr>=55?{icon:"🎯",color:"#10b981",text:`نسبة فوز قوية ${wr}% على ${c.count} صفقة — التنفيذ يعكس التزام جيد بالخطة.`}
                : wr>=40?{icon:"🎯",color:"#f59e0b",text:`نسبة فوز ${wr}% على ${c.count} صفقة — ضمن النطاق المقبول، يحتاج تحسين نسبة الريسك/العائد.`}
                : {icon:"🎯",color:"#ef4444",text:`نسبة فوز منخفضة ${wr}% على ${c.count} صفقة — راجع شروط الدخول والخروج بدقة.`},
          analysis: pf>=1.5?{icon:"🔬",color:"#10b981",text:`Profit Factor ${c.profitFactor} — الأرباح تفوق الخسائر بوضوح، إدارة مخاطر سليمة.`}
                  : pf>=1?{icon:"🔬",color:"#f59e0b",text:`Profit Factor ${c.profitFactor} — قريب من التعادل، أي تحسّن بجودة الصفقات ينعكس مباشرة بالنتيجة.`}
                  : {icon:"🔬",color:"#ef4444",text:`Profit Factor ${c.profitFactor} أقل من 1 — الخسائر تفوق الأرباح، يحتاج تدخل فوري بالاستراتيجية.`},
          psychology: critical===0?{icon:"🧠",color:"#10b981",text:`لا توجد تحذيرات نفسية حرجة — انضباط ذهني جيد أثناء التنفيذ.`}
                    : {icon:"🧠",color:"#ef4444",text:`${critical} تحذير نفسي حرج مسجّل — الجانب النفسي هو الحلقة الأضعف حاليًا ويستحق أولوية.`},
        };
        const ins=M[tab];
        if(!ins)return null;
        return(
          <div style={{padding:"13px 28px",background:`linear-gradient(135deg,${ins.color}1c,#100c07)`,borderBottom:`1px solid ${ins.color}2e`,display:"flex",alignItems:"center",gap:11,position:"relative",zIndex:1}}>
            <span className="pulse-slow" style={{fontSize:16,flexShrink:0}}>{ins.icon}</span>
            <p style={{fontSize:12.5,color:"rgba(255,255,255,0.78)",fontWeight:600,lineHeight:1.5,margin:0}}>{ins.text}</p>
          </div>
        );
      })()}
      ...
      <div style={{padding:"24px 28px 60px",position:"relative",zIndex:1}}>
        ...
        {tab==="overview"&&(()=>{
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1.6fr 1fr",gap:16}}>
              <div style={{background:"#171310",backdropFilter:"blur(8px)",border:"1px solid #3A2E1F",borderRadius:20,padding:"20px 22px",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              <div style={{background:"#171310",backdropFilter:"blur(8px)",border:"1px solid #3A2E1F",borderRadius:20,padding:"20px 22px",display:"flex",gap:18,alignItems:"center",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              <div style={{background:"#171310",backdropFilter:"blur(8px)",border:"1px solid #3A2E1F",borderRadius:20,padding:"20px 22px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1.3fr 1fr",gap:16}}>
              {Object.keys(c.sessions||{}).length>0&&(
                <div style={{background:"#171310",backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",border:"1px solid #3A2E1F",borderRadius:18,padding:"20px 22px",boxShadow:"0 8px 32px rgba(0,0,0,0.25)",transition:"transform .4s cubic-bezier(.4,0,.2,1),box-shadow .4s ease,border-color .4s ease"}}>
            ...
            {/* ══════ SECTION 1: الأداء العام — Big Number Cards ══════ */}
            <div>
              <SectionHead icon="🏁" title="الأداء العام"/>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:14}}>
                <BigNumberCard icon="📋" label="إجمالي الصفقات" value={c.count} color="#FCD34D"/>
                <BigNumberCard icon="💰" label="صافي الربح/الخسارة" value={(c.total>=0?"+":"")+"$"+c.total?.toFixed(0)} color={c.total>=0?"#10b981":"#ef4444"} arrow={c.total>=0?1:-1}/>
                <BigNumberCard icon="🎯" label="نسبة الفوز" value={c.wr+"%"} color={c.wr>=60?"#10b981":c.wr>=40?"#f59e0b":"#ef4444"} progress={c.wr}/>
                <div className="card-hover breathe-box" style={{"--glow":`${scoreCol}40`,background:"#171310",border:`1px solid ${scoreCol}30`,borderRadius:18,padding:"14px",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <ScoreArc score={c.score} size={78}/>
                </div>
              </div>
            </div>

            {/* ══════ SECTION 1.5: الاتجاه والتوقعات ══════ */}
            <div>
              <SectionHead icon="🔮" title="اتجاه وتوقع الأرباح"/>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1.3fr",gap:14}}>
                <ProfitArrowChart equityCurve={c.equityCurve}/>
                <ProfitProjectionChart c={c}/>
              </div>
            </div>

            {/* ══════ SECTION 2: تحليل الأرباح والخسائر — Bar Charts ══════ */}
            <div>
              <SectionHead icon="📊" title="تحليل الأرباح والخسائر"/>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14}}>
                <DualBar title="أكبر ربح مقابل أكبر خسارة" aLabel="أكبر ربح" aVal={c.largestWin||0} aColor="#10b981" bLabel="أكبر خسارة" bVal={c.largestLoss||0} bColor="#ef4444"/>
                <DualBar title="إجمالي الربح مقابل إجمالي الخسارة" aLabel="إجمالي ربح" aVal={c.winTotal||0} aColor="#10b981" bLabel="إجمالي خسارة" bVal={c.lossTotal||0} bColor="#ef4444"/>
                <DualBar title="متوسط الربح مقابل متوسط الخسارة" aLabel="متوسط ربح" aVal={c.avgWin||0} aColor="#10b981" bLabel="متوسط خسارة" bVal={c.avgLoss||0} bColor="#ef4444"/>
              </div>
            </div>

            {/* ══════ SECTION 3: الكفاءة والنسب — Gauges ══════ */}
            <div>
              <SectionHead icon="⚙️" title="مؤشرات الكفاءة والنسب"/>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:14,alignItems:"stretch"}}>
                <div style={{background:"#171310",border:"1px solid #3A2E1F",borderRadius:18,padding:"12px",display:"flex",justifyContent:"center"}}>
                  <RadialGauge value={Number(c.profitFactor)||0} max={Math.max(3,Number(c.profitFactor)||0)} label="Profit Factor" color={Number(c.profitFactor)>=1.5?"#10b981":Number(c.profitFactor)>=1?"#f59e0b":"#ef4444"}/>
                </div>
                <div style={{background:"#171310",border:"1px solid #3A2E1F",borderRadius:18,padding:"12px",display:"flex",justifyContent:"center"}}>
                  <RadialGauge value={Number(c.avgRR)||0} max={Math.max(3,Number(c.avgRR)||0)} label="متوسط RR" color="#FCD34D" unit=":1"/>
                </div>
                <BigNumberCard icon="📐" label="Expectancy / صفقة" value={"$"+(c.expectancy?.toFixed(2)??0)} color={c.expectancy>=0?"#10b981":"#ef4444"}/>
                <BigNumberCard icon="⚖️" label="نسبة ربح/خسارة" value={c.winLossRatio} color="#FB923C"/>
              </div>
            </div>

            {/* ══════ SECTION 4: المخاطر والتراجع — Area Chart ══════ */}
            <div>
              <SectionHead icon="🌊" title="تحليل المخاطر والتراجع"/>
              <div style={{background:"#171310",border:"1px solid #3A2E1F",borderRadius:18,padding:"18px 20px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <span style={{fontSize:11,color:"#8A7B60",fontWeight:700}}>أقصى تراجع (Max Drawdown)</span>
                  <span style={{fontSize:16,fontWeight:900,color:c.maxDrawdown>0?"#ef4444":"#10b981",fontFamily:"'JetBrains Mono',monospace"}}>{c.maxDrawdownPct}% · ${c.maxDrawdown?.toFixed(0)}</span>
                </div>
                <DrawdownArea equityCurve={c.equityCurve} h={110}/>
              </div>
            </div>

            {/* ══════ SECTION 5: تحليل السلاسل — Streak Counters ══════ */}
            <div>
              <SectionHead icon="🔥" title="تحليل السلاسل"/>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14}}>
                <StreakCounter icon="🏆" label="أطول سلسلة فوز متتالية" value={c.winStreak} color="#FCD34D"/>
                <StreakCounter icon="❌" label="أطول سلسلة خسارة متتالية" value={c.lossStreak} color="#ef4444"/>
              </div>
            </div>

            {/* ══════ SECTION 6: تفاصيل الصفقات وسلوك السوق ══════ */}
            <div>
              <SectionHead icon="🧩" title="تفاصيل الصفقات وسلوك السوق"/>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                <div style={{background:"#171310",border:"1px solid #3A2E1F",borderRadius:16,padding:"16px 18px",display:"flex",flexDirection:"column",gap:12,justifyContent:"center"}}>
                  <ProgressRow label="نسبة فوز — النصف الأول" value={c.wrFirst} color={c.wrFirst>=50?"#10b981":"#6b7280"}/>
                  <ProgressRow label="نسبة فوز — النصف الثاني" value={c.wrSecond} color={c.wrSecond>=50?"#10b981":"#6b7280"}/>
                </div>
                <SegmentBar title="صفقات شراء (Long) مقابل بيع (Short)" segments={[{label:"Long",value:c.longCount||0,color:"#3b82f6"},{label:"Short",value:c.shortCount||0,color:"#ef4444"}]}/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12}}>
                <MetricChip icon="⚠️" label="صفقات بدون Stop Loss" value={c.noSLCount} color={c.noSLCount>0?"#ef4444":"#6b7280"}/>
                <MetricChip icon="✅" label="متوسط الشروط المحققة" value={c.avgConditions} color="#FCD34D"/>
                <MetricChip icon="🪙" label="حجم التداول الكلي" value={"$"+(c.totalVolume?.toFixed(0)??0)} color="#F59E0B"/>
                <MetricChip icon="📅" label="صفقات/أسبوع" value={c.tradesPerWeek} color="#FB923C"/>
                <MetricChip icon="📸" label="عدد الصور المرفقة" value={c.imageCount} color="#a78bfa"/>
              </div>
            </div>
          </div>
          );})()}
        ...
        {tab==="trades"&&(
          <div>
            ...
            {sorted.filter(t=>t.image).length>0&&(
              <div style={{marginBottom:22,background:"linear-gradient(135deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))",border:"1px solid #3A2E1F",borderRadius:16,padding:"16px 18px"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:10}}>
                  {sorted.filter(t=>t.image).map((t,i)=>(
                    <div key={i} onClick={()=>setTradeSel(t)} className="card-hover" style={{borderRadius:10,overflow:"hidden",border:"1px solid #3A2E1F",position:"relative"}}>
            ...
            {/* ── FULL TRADES TABLE — every field visible ── */}
            <div style={{background:"#171310",border:"1px solid #3A2E1F",borderRadius:16,overflow:"hidden"}}>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",minWidth:920,fontSize:12}}>
                  <thead>
                    <tr style={{background:"#17130F"}}>
                      {["#","التاريخ","الرمز","اتجاه","PnL","RR","SL","TP","درجة","الإعداد","الجلسة","أخطاء","شروط","صورة"].map((h,i)=>(
                        <th key={i} style={{padding:"10px 12px",fontSize:9,color:"#8A7B60",fontWeight:700,textTransform:"uppercase",letterSpacing:".06em",whiteSpace:"nowrap",borderBottom:"1px solid #3A2E1F",textAlign:i===0?"center":"right"}}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {sorted.length===0&&<tr><td colSpan={14} style={{textAlign:"center",padding:50,color:"rgba(255,255,255,0.25)"}}>لا توجد صفقات</td></tr>}
                    {sorted.map((t,i)=>{
                      const isW=t.pnl>=0;const gc=GC[t.grade]||(isW?"#10b981":"#ef4444");
                      return(
                        <tr key={i} className="row-hover" onClick={()=>setTradeSel(t)}
                          style={{background:i%2===0?"#17130F":"#1D1610",borderBottom:"1px solid #241C13",cursor:"pointer"}}>
                          <td style={{padding:"9px 12px",textAlign:"center",color:"#8A7B60",fontFamily:"'JetBrains Mono',monospace"}}>{i+1}</td>
                          <td style={{padding:"9px 12px",color:"#B8A98C",whiteSpace:"nowrap"}}>{t.date}</td>
                          <td style={{padding:"9px 12px",fontWeight:800,color:"#F1E9D8",fontFamily:"'JetBrains Mono',monospace",whiteSpace:"nowrap"}}>{t.symbol}</td>
                          <td style={{padding:"9px 12px",fontWeight:700,color:t.direction==="L"?"#10b981":"#ef4444"}}>{t.direction==="L"?"LONG":"SHORT"}</td>
                          <td style={{padding:"9px 12px",fontWeight:900,color:isW?"#10b981":"#ef4444",fontFamily:"'JetBrains Mono',monospace"}}>{isW?"+":""}${t.pnl}</td>
                          <td style={{padding:"9px 12px",color:"#f59e0b",fontFamily:"'JetBrains Mono',monospace"}}>{t.rr||0}:1</td>
                          <td style={{padding:"9px 12px",color:t.sl?"#B8A98C":"#ef4444",fontFamily:"'JetBrains Mono',monospace"}}>{t.sl||"—"}</td>
                          <td style={{padding:"9px 12px",color:"#B8A98C",fontFamily:"'JetBrains Mono',monospace"}}>{t.tp||"—"}</td>
                          <td>{t.grade?<span style={{background:`${gc}18`,color:gc,border:`1px solid ${gc}35`,borderRadius:6,padding:"2px 8px",fontSize:11,fontWeight:800}}>{t.grade}</span>:<span style={{color:"rgba(255,255,255,0.2)"}}>—</span>}</td>
                          <td style={{padding:"9px 12px",color:"#B8A98C",whiteSpace:"nowrap"}}>{t.setup||"—"}</td>
                          <td style={{padding:"9px 12px",color:"#B8A98C",whiteSpace:"nowrap"}}>{t.session||"—"}</td>
                          <td style={{padding:"9px 12px",color:t.mistakes?.length?"#ef4444":"rgba(255,255,255,0.2)",textAlign:"center"}}>{t.mistakes?.length||0}</td>
                          <td style={{padding:"9px 12px",color:"#B8A98C",textAlign:"center"}}>{t.conditions?.length||0}</td>
                          <td style={{padding:"9px 12px",textAlign:"center"}}>{t.image?"🖼️":"—"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
        ...
        {tab==="analysis"&&(()=>{
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            {(()=>{
              const byMonth={};
              (c.trades||[]).forEach(t=>{if(t.date){const m=t.date.slice(0,7);if(!byMonth[m])byMonth[m]={pnl:0,n:0};byMonth[m].pnl+=t.pnl||0;byMonth[m].n++;}});
              const months=Object.entries(byMonth).sort((a,b)=>a[0].localeCompare(b[0]));
              if(!months.length)return null;
              const maxAbs=Math.max(...months.map(([,d])=>Math.abs(d.pnl)),1);
              return(
              <div className="whoop-card" style={{padding:"20px 22px"}}>
                <p style={{fontSize:10,color:"rgba(255,255,255,0.35)",fontWeight:700,textTransform:"uppercase",letterSpacing:".1em",marginBottom:16}}>🗓️ الأداء الشهري</p>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {months.map(([m,d])=>{
                    const pos=d.pnl>=0;const w=Math.min(100,Math.abs(d.pnl)/maxAbs*100);
                    return(
                      <div key={m} style={{display:"grid",gridTemplateColumns:"70px 1fr 90px",gap:12,alignItems:"center"}}>
                        <span style={{fontSize:11,color:"rgba(255,255,255,0.5)",fontFamily:"'JetBrains Mono',monospace",fontWeight:700}}>{m}</span>
                        <div style={{height:8,background:"rgba(255,255,255,0.05)",borderRadius:4,overflow:"hidden"}}>
                          <div className="progress-fill" style={{"--target-w":w+"%",height:8,width:w+"%",borderRadius:4,background:pos?"linear-gradient(90deg,#059669,#10b981)":"linear-gradient(90deg,#dc2626,#ef4444)",boxShadow:`0 0 10px ${pos?"#10b98170":"#ef444470"}`}}/>
                        </div>
                        <span style={{fontSize:12,fontWeight:800,color:pos?"#10b981":"#ef4444",fontFamily:"'JetBrains Mono',monospace",textAlign:"left"}}>{pos?"+":""}${d.pnl.toFixed(0)} <span style={{color:"rgba(255,255,255,0.3)",fontWeight:500}}>· {d.n}</span></span>
                      </div>
                    );
                  })}
                </div>
        ...
        {tab==="psychology"&&(()=>{
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            {/* ── WHOOP RINGS ROW ── */}
            <div style={{background:"#171310",backdropFilter:"blur(8px)",border:"1px solid #3A2E1F",borderRadius:24,padding:"24px",position:"relative",overflow:"hidden"}}>
          ...
          <div onClick={()=>setKpiSel(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
            <div onClick={e=>e.stopPropagation()} className="modal-glass" style={{width:"100%",maxWidth:640,maxHeight:"92vh",overflowY:"auto",animation:"popIn .2s ease"}}>
              ...
              <div style={{background:`linear-gradient(135deg,${kpiSel.color}15,rgba(10,7,0,0.9))`,borderBottom:`1px solid ${kpiSel.color}20`,padding:"24px 26px",borderRadius:"24px 24px 0 0",position:"relative",overflow:"hidden"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:16,position:"relative"}}>
                  ...
                  <button onClick={()=>setKpiSel(null)}
                    style={{background:"rgba(255,255,255,0.06)",border:"1px solid #3A2E1F",borderRadius:10,width:34,height:34,color:"rgba(255,255,255,0.5)",cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",transition:"all .2s"}}
              ...
              <div style={{padding:"22px 24px",display:"flex",flexDirection:"column",gap:18}}>
                ...
                <div style={{background:"rgba(255,255,255,0.03)",backdropFilter:"blur(10px)",border:"1px solid #3A2E1F",borderRadius:16,padding:"16px 18px",position:"relative",overflow:"hidden"}}>
                  ...
                  {info.chart==="rmultiple"&&(
                    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                      {[{lbl:"3R+",n:c.rMultiples?.r3,col:"#10b981"},{lbl:"2R–3R",n:c.rMultiples?.r2,col:"#34d399"},{lbl:"1R–2R",n:c.rMultiples?.r1,col:"#FCD34D"},{lbl:"خسارة",n:c.rMultiples?.rNeg,col:"#ef4444"}].map(x=>(
                        <div key={x.lbl} style={{textAlign:"center",background:"rgba(255,255,255,0.03)",backdropFilter:"blur(8px)",border:"1px solid #3A2E1F",borderRadius:12,padding:"14px 6px",transition:"all .2s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=x.col+"40";e.currentTarget.style.transform="translateY(-2px)";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(255,255,255,0.06)";e.currentTarget.style.transform="";}}>
                  ...
                  {info.chart==="gallery"&&(
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(90px,1fr))",gap:8}}>
                      {sorted.filter(t=>t.image).slice(0,8).map((t,i)=>(
                        <img key={i} src={t.image} alt="" style={{width:"100%",height:60,objectFit:"cover",borderRadius:8,border:"1px solid #3A2E1F"}}/>
                      ))}
...
/* ── Error Boundary — تمنع الشاشة البيضاء وتعرض سبب الخطأ ── */
class ProfileErrorBoundary extends React.Component{
  constructor(props){super(props);this.state={error:null};}
  static getDerivedStateFromError(error){return{error};}
  componentDidCatch(error,info){console.error("ProfilePage crashed:",error,info);}
  render(){
    if(this.state.error){
      return(
        <div style={{minHeight:"100vh",background:"#0C0A08",color:"#fff",fontFamily:"'Outfit',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,padding:24,textAlign:"center"}}>
          <div style={{fontSize:50}}>⚠️</div>
          <p style={{fontSize:18,fontWeight:800,color:"#f87171"}}>حصل خطأ أثناء عرض بيانات هذا العميل</p>
          <p style={{fontSize:13,color:"rgba(255,255,255,0.5)",maxWidth:520,fontFamily:"monospace",direction:"ltr"}}>{String(this.state.error?.message||this.state.error)}</p>
          <p style={{fontSize:12,color:"rgba(255,255,255,0.35)"}}>الأغلب سبب الخطأ بيانات ناقصة أو غير سليمة بحساب هذا العميل تحديداً (مثال: حقل trades غير محفوظ كـ array).</p>
          <button className="btn-hover" onClick={this.props.onBack} style={{background:"rgba(217,119,6,0.15)",color:"#FCD34D",border:"1px solid rgba(217,119,6,0.3)",borderRadius:12,padding:"10px 24px",cursor:"pointer",fontWeight:700,fontFamily:"inherit"}}>رجوع لقائمة العملاء</button>
        </div>
      );
    }
...
export default function AdminPage({user,onBack}){
  ...
  async function loadClients(){
    ...
    try{
      ...
      snap.forEach(doc=>{
        const d=doc.data();
        const safeTrades=Array.isArray(d.trades)?d.trades:[];
        const safeMotivations=Array.isArray(d.motivations)?d.motivations:[];
        const stats=calcStats(safeTrades);
        list.push({uid:doc.id,name:d.name||"بدون اسم",email:d.email||"",phone:d.phone||"",createdAt:d.createdAt?.toDate?.()||null,trades:safeTrades,motivations:safeMotivations,...stats});
      });
      setClients(list);
  ...
  if(selected)return(
    <ProfileErrorBoundary onBack={()=>setSelected(null)}>
      <ProfilePage client={selected} onBack={()=>setSelected(null)}/>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0C0A08",color:"#fff",fontFamily:"'Outfit',sans-serif",direction:"rtl",position:"relative"}}>
      ...
      <div style={{background:"linear-gradient(180deg,#170F08 0%,#120A04 100%)",borderBottom:"1px solid rgba(217,119,6,0.08)",position:"relative",zIndex:1}}>
        {/* top bar */}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 28px",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            <div className="pulse-slow" style={{width:42,height:42,borderRadius:12,background:"linear-gradient(135deg,#D97706,#F59E0B)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,boxShadow:"0 0 20px rgba(217,119,6,0.4)"}}>🛡️</div>
            <div>
      ...
      {/* ── SEARCH + CONTROLS ── */}
      <div style={{padding:"16px 28px",background:"rgba(0,0,0,0.3)",borderBottom:"1px solid rgba(255,255,255,0.04)",display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",position:"relative",zIndex:1}}>
        <div style={{flex:1,minWidth:200,position:"relative"}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ابحث باسم أو إيميل..."
            style={{width:"100%",background:"rgba(255,255,255,0.03)",border:"1px solid #3A2E1F",borderRadius:10,padding:"9px 40px 9px 14px",color:"#fff",fontSize:13,fontFamily:"inherit"}}/>
        </div>

        <div style={{display:"flex",gap:6}}>
          {[
            {k:"score",l:"Score",ic:<svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M13 2 4 14h6l-1 8 9-12h-6l1-8z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" fill="currentColor" fillOpacity="0.15"/></svg>},
            {k:"wr",l:"WR",ic:<svg width="13" height="13" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="1.8"/><circle cx="12" cy="12" r="4.5" stroke="currentColor" strokeWidth="1.8"/><circle cx="12" cy="12" r="1" fill="currentColor"/></svg>},
            {k:"trades",l:"Trades",ic:<svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M4 19V11M12 19V5M20 19v-6" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"/></svg>},
            {k:"name",l:"Name",ic:<svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M6 4v16M6 4l4 4M6 4 2 8M14 6h8M14 12h8M14 18h5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>},
          ].map(({k,l,ic})=>(
            <button key={k} className="btn-hover" onClick={()=>setSort(k)}
              style={{display:"flex",alignItems:"center",gap:6,background:sort===k?"linear-gradient(135deg,rgba(217,119,6,0.28),#1D1610)":"#171310",color:sort===k?"#FCD34D":"rgba(255,255,255,0.4)",border:`1px solid ${sort===k?"rgba(217,119,6,0.4)":"rgba(255,255,255,0.08)"}`,borderRadius:10,padding:"8px 15px",fontSize:12,fontWeight:700,letterSpacing:".02em",fontFamily:"inherit",whiteSpace:"nowrap"}}>
              <span style={{display:"flex",filter:sort===k?"drop-shadow(0 0 4px #FCD34Daa)":"none"}}>{ic}</span>
              {l}
            </button>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:4}}>
          {[
            {v:"grid",ic:<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="8" height="8" rx="1.5" stroke="currentColor" strokeWidth="2"/><rect x="13" y="3" width="8" height="8" rx="1.5" stroke="currentColor" strokeWidth="2"/><rect x="3" y="13" width="8" height="8" rx="1.5" stroke="currentColor" strokeWidth="2"/><rect x="13" y="13" width="8" height="8" rx="1.5" stroke="currentColor" strokeWidth="2"/></svg>},
            {v:"list",ic:<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><rect x="3" y="4" width="18" height="4" rx="1.2" stroke="currentColor" strokeWidth="1.8"/><rect x="3" y="10" width="18" height="4" rx="1.2" stroke="currentColor" strokeWidth="1.8"/><rect x="3" y="16" width="18" height="4" rx="1.2" stroke="currentColor" strokeWidth="1.8"/></svg>},
          ].map(({v,ic})=>(
            <button key={v} className="btn-hover" onClick={()=>setView(v)} style={{display:"flex",alignItems:"center",justifyContent:"center",background:view===v?"linear-gradient(135deg,rgba(217,119,6,0.28),#1D1610)":"#171310",color:view===v?"#FCD34D":"rgba(255,255,255,0.35)",border:`1px solid ${view===v?"rgba(217,119,6,0.4)":"rgba(255,255,255,0.08)"}`,borderRadius:10,padding:"8px 12px",fontFamily:"inherit"}}>
              {ic}
            </button>
          ))}
          <button className="btn-hover" onClick={loadClients} style={{display:"flex",alignItems:"center",justifyContent:"center",background:"#171310",color:"rgba(255,255,255,0.4)",border:"1px solid #3A2E1F",borderRadius:10,padding:"8px 12px",fontFamily:"inherit"}}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M4 12a8 8 0 0 1 14-5.3M20 12a8 8 0 0 1-14 5.3" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><path d="M18 3v5h-5M6 21v-5h5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </button>
        </div>
      </div>

      {/* ── CONTENT ── */}
      <div style={{padding:"20px 28px 80px",position:"relative",zIndex:1}}>
        {loading&&(
        ...
        {view==="grid"&&(
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:16}}>
            {filtered.map((c,idx)=>{
              ...
              return(
                <div key={c.uid} className="card-hover glow-pulse" onClick={()=>setSelected(c)}
                  style={{"--glow":`${sc}45`,background:"#171310",backgroundImage:`radial-gradient(circle at 15% 10%,${sc}0c,transparent 55%)`,backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",border:`1px solid ${sc}2e`,borderRadius:20,padding:"20px",overflow:"hidden",position:"relative",animation:`fadeUp .35s ease ${idx*0.04}s both`}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                    <div style={{display:"flex",alignItems:"center",gap:12}}>
                      <div className="avatar-glow" style={{width:48,height:48,borderRadius:14,background:`linear-gradient(135deg,${sc}25,${sc}08)`,border:`1.5px solid ${sc}35`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,fontWeight:900,color:sc,flexShrink:0,fontFamily:"'JetBrains Mono',monospace"}}>
                        {(c.name||"?")[0].toUpperCase()}
                      </div>
                      <div>
        ...
        {/* LIST */}
        {view==="list"&&(
          <div style={{background:"#171310",backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",border:"1px solid #3A2E1F",borderRadius:18,overflow:"hidden",boxShadow:"0 12px 40px rgba(0,0,0,0.3)"}}>
            ...
            {filtered.map((c,idx)=>{
              ...
              return(
                <div key={c.uid} className="row-hover" onClick={()=>setSelected(c)}
                  ...
                  <div className="avatar-glow" style={{"--glow":`${sc}55`,width:38,height:38,borderRadius:10,background:`linear-gradient(135deg,${sc}28,${sc}0c)`,border:`1px solid ${sc}35`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,fontWeight:900,color:sc,fontFamily:"'JetBrains Mono',monospace"}}>
                    {(c.name||"?")[0].toUpperCase()}
                  </div>
                  <div>
