...
function calcStats(trades = []) {
  if (!trades.length) return { score:0,wr:0,total:0,count:0,avgRR:"0.0",bestDay:null,worstDay:null,topMistakes:[],topSetups:[],longCount:0,shortCount:0,avgPnl:0,winStreak:0,lossStreak:0,profitFactor:"0.00",sessions:{},gradeMap:{},winTotal:0,lossTotal:0,avgConditions:0,weakConditions:[],strongConditions:[],symbolStats:[],setupStats:[],sessionStats:[],dowStats:[],equityCurve:[] };
  ...
  return {score,wr,total,count:trades.length,avgRR,bestDay,worstDay,topMistakes,topSetups,longCount:trades.filter(t=>t.direction==="L").length,shortCount:trades.filter(t=>t.direction==="S").length,avgPnl,winStreak:ws,lossStreak:ls,profitFactor,sessions,gradeMap,winTotal,lossTotal:Math.abs(losses.reduce((s,t)=>s+t.pnl,0)),avgConditions,weakConditions,strongConditions,symbolStats,setupStats,sessionStats,dowStats,equityCurve};
}
