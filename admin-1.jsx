...
function EquityCone({trades,w=140,h=48}){
  ...
  pts.forEach((v,i)=>{
    const x=(i/(n-1))*w;
    const envelope=0.1+0.9*(i/(n-1));           // narrow start → wide mouth (funnel shape)
    const norm=(v-mn)/rng;
    const wobble=(norm-0.5)*0.5;                  // subtle organic ride along the funnel edge
    const half=Math.max(1,(mid-2)*envelope*(1+wobble*0.4));
    top.push([x,mid-half]);
    bot.push([x,mid+half]);
  });
  const topPath=top.map(([x,y],i)=>`${i===0?"M":"L"} ${x.toFixed(1)},${y.toFixed(1)}`).join(" ");
  const botPath=[...bot].reverse().map(([x,y])=>`L ${x.toFixed(1)},${y.toFixed(1)}`).join(" ");
  ...
  return(
    <svg width={w} height={h} style={{display:"block",overflow:"visible"}}>
      ...
      <path d={`${topPath} ${botPath} Z`} fill={`url(#${gid})`} stroke={col} strokeOpacity="0.55" strokeWidth="1.2"
        className="breathe" style={{"--glow":`${col}70`}}/>
      <circle cx={w-2} cy={mid} r="3" fill={col} className="pulse-slow" style={{color:col}}/>
    </svg>
  );
}
...
function ProfilePage({client:c,onBack}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0C0A08",color:"#fff",fontFamily:"'Outfit',sans-serif",direction:"rtl",position:"relative"}}>
      ...
      {/* TABS */}
      <div style={{display:"flex",gap:8,background:"rgba(0,0,0,0.35)",borderBottom:"1px solid rgba(255,255,255,0.05)",padding:"12px 28px",overflowX:"auto",position:"relative",zIndex:1}}>
        {[
          {id:"overview",label:"Overview",color:"#F59E0B",icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M4 20V11M12 20V4M20 20v-8" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"/></svg>},
          {id:"trades",label:`Trades (${c.count})`,color:"#FCD34D",icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><rect x="5" y="3" width="14" height="18" rx="2.5" stroke="currentColor" strokeWidth="2"/><path d="M9 8.5h6M9 12.5h6M9 16.5h3" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>},
          {id:"analysis",label:"Analysis",color:"#FB923C",icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><circle cx="10.5" cy="10.5" r="6.5" stroke="currentColor" strokeWidth="2.2"/><path d="M20 20l-4.4-4.4" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"/></svg>},
          {id:"psychology",label:"Psychology",color:"#D97706",icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M9.5 3.5A3.5 3.5 0 0 0 6 7v.6a2.6 2.6 0 0 0-.8 4.9v1a3.5 3.5 0 0 0 3.5 3.5H9M14.5 3.5A3.5 3.5 0 0 1 18 7v.6a2.6 2.6 0 0 1 .8 4.9v1a3.5 3.5 0 0 1-3.5 3.5H15M9.5 3.5v13M14.5 3.5v13" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/></svg>},
          ...
          return(
            <button key={id} className={`tab-item${active?" active":""}`} onClick={()=>setTab(id)}
              style={{display:"flex",alignItems:"center",gap:8,padding:"10px 18px",borderRadius:14,
                border:active?`1px solid ${color}4d`:"1px solid rgba(255,255,255,0.06)",
                background:active?`linear-gradient(135deg,${color}26,${color}0a)`:"rgba(255,255,255,0.025)",
                backdropFilter:"blur(14px)",WebkitBackdropFilter:"blur(14px)",
                fontSize:13,fontWeight:700,color:active?color:"rgba(255,255,255,0.4)",whiteSpace:"nowrap",fontFamily:"inherit"}}>
              <span className="tab-ico" style={{display:"flex",color:active?color:"rgba(255,255,255,0.35)",filter:active?`drop-shadow(0 0 6px ${color}aa)`:"none"}}>{icon}</span>
              {label}
            </button>
          );
        })}
    ...
    <div style={{minHeight:"100vh",background:"#0C0A08",color:"#fff",fontFamily:"'Outfit',sans-serif",direction:"rtl",position:"relative"}}>
      ...
      <div style={{padding:"16px 28px",background:"rgba(0,0,0,0.3)",borderBottom:"1px solid rgba(255,255,255,0.04)",display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"flex",gap:4}}>
          {[["grid","⊞"],["list","☰"]].map(([v,l])=>(
            <button key={v} className="btn-hover" onClick={()=>setView(v)} style={{background:view===v?"rgba(217,119,6,0.2)":"rgba(255,255,255,0.03)",color:view===v?"#FCD34D":"rgba(255,255,255,0.35)",border:`1px solid ${view===v?"rgba(217,119,6,0.3)":"rgba(255,255,255,0.06)"}`,borderRadius:8,padding:"8px 12px",fontSize:14,fontFamily:"inherit"}}>
              {l}
            </button>
          ))}
