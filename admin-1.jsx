...
function ProfilePage({client:c,onBack,onDelete}){
  ...
  const handleDelete=async()=>{
    ...
    try{
      ...
      setDelError(e?.code==="permission-denied"
        ? "تم رفض العملية من صلاحيات Firestore (Security Rules) — تأكد أن قواعد الأمان تسمح للأدمن بالحذف"
  ...
  const kpis=[
    ...
    {icon:"🏆",label:"Largest Win",value:"$"+c.largestWin?.toFixed(0),color:"#10b981",sub:"Best single trade",delay:.6},
    {icon:"💔",label:"Largest Loss",value:"$"+c.largestLoss?.toFixed(0),color:"#ef4444",sub:"Worst single trade",delay:.65},
    {icon:"⚔️",label:"Win/Loss Ratio",value:c.winLossRatio,color:Number(c.winLossRatio)>=1.5?"#10b981":Number(c.winLossRatio)>=1?"#f59e0b":"#ef4444",sub:"Avg win ÷ avg loss",delay:.7},
    {icon:"📦",label:"Total Volume",value:c.totalVolume?.toFixed(2)+" lot",color:"#F59E0B",sub:"Sum of all lots",delay:.75},
    {icon:"🗓️",label:"Trades/Week",value:c.tradesPerWeek,color:"#FB923C",sub:"Trading frequency",delay:.8},
    {icon:"📸",label:"Evidence Rate",value:c.count?Math.round(c.imageCount/c.count*100)+"%":"0%",color:"#FCD34D",sub:c.imageCount+" صورة مرفقة",delay:.85},
    {icon:"🚫",label:"No Stop-Loss",value:c.noSLCount,color:c.noSLCount>0?"#ef4444":"#10b981",sub:"صفقات بدون SL",delay:.9},
  ];
